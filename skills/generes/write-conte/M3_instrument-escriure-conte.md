...
| 7 Diàleg — Atribució i funció | `binary` (per existència) + `structural` | no | detectar marques de diàleg ("va dir", "va preguntar"); verificar atribució per nom (A2+). A A1, diàleg és opcional: no penalitzar l'absència ni incloure'l a l'autoavaluació del derivat |
